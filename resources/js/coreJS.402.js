...
//FIXME:quizá con extend?
	...
	function $ajax(url,params,callbacks){
		...
		if(params){switch(true){
			...
			case ($is.element(params) && params.tagName && params.tagName == 'FORM'):data = new FormData(params);break;
			default:data = params;
		}}
	...
	var $fx = {
		height: function(el,height,callback,time,interval){
			if(!el.$B){el = $fix(el);}
			el.$B({'.height':el.offsetHeight+'px'});
			setTimeout(function(){el.$B({'.transition':'height .5s ease','.height':height+'px'});},50);
			setTimeout(function(){el.$B({'.transition':'height 0 ease 0'});},60);
		},
		enter: function(el,params){
			if(!el.$B){el = $fix(el);}
			setTimeout(function(){el.$B({'.transition':'opacity .5s ease','.opacity':1,'.overflow':'hidden'});},100);
			setTimeout(function(){el.$B({'.transition':'opacity 0 ease 0'});if(params && params.callback){params.callback(el);}},610);
		},
		leave: function(el,params){
			if(!el.$B){el = $fix(el);}
			setTimeout(function(){el.$B({'.transition':'opacity .5s ease','.opacity':0,'.overflow':'hidden'});},100);
			setTimeout(function(){el.$B({'.transition':'opacity 0 ease 0'});if(params && params.callback){params.callback(el);}},610);
		},
		leaveVertical: function(el,params){
			if(!el.$B){el = $fix(el);}
			el.$B({'.height':el.offsetHeight+'px'});
			setTimeout(function(){el.$B({'.transition':'opacity .5s ease,height .5s ease','.opacity':0,'.overflow':'hidden','.height':0});},60);
			setTimeout(function(){el.$B({'.transition':'opacity 0 ease 0,height 0 ease 0'});if(params && params.callback){params.callback(el);}},570);
		}
	};
	var $transition = {
		setHTMLByState: function(p,s,html){
			var o = p.firstChild;while(o.nextSibling && (!o.getAttribute || o.getAttribute('data-state') != s)){o = o.nextSibling;}
			if(!o.getAttribute || o.getAttribute('data-state') != s){return false;}
			o.innerHTML = html;
		},
		toState: function(p,s,cb){
			/* p (parent) & s (state) */
			p.setAttribute('data-ready','false');
			var o = p.firstChild;while(o.nextSibling && (!o.getAttribute || o.getAttribute('data-state') != s)){o = o.nextSibling;}
			if(!o.getAttribute || o.getAttribute('data-state') != s){return false;}
			var c = p.getAttribute('data-state');
			if(c){var l = c;var c = p.firstChild;while(c.nextSibling && (!c.getAttribute || c.getAttribute('data-state') != l)){c = c.nextSibling;}if(!c.getAttribute || c.getAttribute('data-state') != l){c = false;}}
			if(!c){$each(p.childNodes,function(k,v){if(v.nodeType != 1){return;}if($getElementStyle(v,'display') == 'block'){c = v;}});}
			if(!c){alert(1);return false;}
			p.setAttribute('data-state',s);

			if(!o.$B){o = $fix(o);}if(!c.$B){c = $fix(c);}
			var parent = {'width':p.offsetWidth,'height':p.offsetHeight,'position':$getOffsetPosition(p),
				'borderTopWidth':parseInt($getElementStyle(p,'border-top-width')),'borderBottomWidth':parseInt($getElementStyle(p,'border-bottom-width')),
				'borderLeftWidth':parseInt($getElementStyle(p,'border-left-width')),
				'paddingTop':parseInt($getElementStyle(p,'padding-top')),'paddingBottom':parseInt($getElementStyle(p,'padding-bottom')),
				'paddingLeft':parseInt($getElementStyle(p,'padding-left')),'paddingRight':parseInt($getElementStyle(p,'padding-right'))
			};

			if(!parent.width){
				/* Si parent.width vale 0, es decir, el elemento no es visible, entonces no necesitamos hacer transiciones */
				if(!c.getAttribute('data-state')){c.parentNode.removeChild(c);}else{c.$B({'.display':'none'});}
				o.$B({'.display':'block','.position':'static','.top':'auto','.left':'auto','.visibility':'visible','.opacity':1});
				if(cb){setTimeout(function(){cb(r);},1300);}
				return true;
			}
			/* o = next, c = current */
			$E.style.apply(p,{'.overflow':'hidden','.boxSizing':'border-box','.width':parent.width+'px','.height':parent.height+'px'});
			$E.style.apply(o,{'.position':'absolute','.display':'block','.visibility':'visible','.opacity':0,'.width':(parent.width-parent.paddingLeft-parent.paddingRight)+'px'});
			$E.style.apply(c,{'.position':'absolute','.width':c.clientWidth+'px'});
			var newHeight = o.clientHeight;
			newHeight += parent.paddingTop+parent.paddingBottom;

			var r = {'elem':o,'parent':p};
			$fx.height(p,newHeight);
			$fx.leave(c,{'callback':function(el){$E.style.apply(c,{'.display':'none'});}});
			$fx.enter(o,{'callback':function(el){
				$E.style.apply(o,{'.position':'static','.top':'auto','.left':'auto','.overflow':'visible'});
				$E.style.apply(p,{'.height':'auto','.overflow':'visible'});
			}});
			setTimeout(function(){p.setAttribute('data-ready','true');if(cb){cb(r);}},1300);
			return r;
		}
	};
